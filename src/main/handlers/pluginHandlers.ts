...
`;
    ...
    // Starship uses TOML with $include directive (array for multiple includes)
    const content = `# MacTheme Starship Configuration
# This file is auto-generated by MacTheme. Customize via starship-overrides.toml instead.

# Include preset and user overrides (order matters - later files override earlier ones)
"$include" = [
  "${currentPresetPath}/starship.toml",
  "${homeDir}/.config/starship-overrides.toml",
]
`;
    await writeFile(path.join(configDir, 'starship.toml'), content);
    logger.info('Generated Starship wrapper config');
  } else if (appName === 'tmux') {
    // tmux uses source-file directive
    const content = `# MacTheme tmux Configuration
# This file is auto-generated by MacTheme. Customize via .tmux-overrides.conf instead.
...
`;
    ...
    if (activePreset) {
      ...
      if (existsSync(presetConfigPath)) {
        ...
        // Prepend MacTheme header
        content = `# MacTheme bat Configuration
# This file is auto-generated by MacTheme.
# Preset: ${activePreset}
