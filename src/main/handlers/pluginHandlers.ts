...
const PLUGIN_DEFINITIONS = {
  ...
  bat: {
    ...
    configDir: path.join(os.homedir(), '.config', 'bat'),
...
# This file is never modified by Flowstate and survives preset switches.
...
`,
    ...
    case 'starship':
      return {
        path: path.join(homeDir, '.config', 'starship-overrides.toml'),
...
`,
    ...
    case 'bat':
      // bat doesn't support includes, so no overrides file
      return null;
    case 'delta':
      // delta uses .gitconfig, overrides go in [delta] section directly
      return null;
    case 'aerospace':
...
export async function handleSetPreset(
  ...
  if (existsSync(mainConfigPath)) {
    const backupPath = `${mainConfigPath}.flowstate-backup`;
  ...
  prefs.pluginConfigs[appName] = {
    mode: 'preset',
    preset: presetName,
    installedBy: existingConfig?.installedBy || 'flowstate',
    configBackupPath: existsSync(`${mainConfigPath}.flowstate-backup`)
      ? `${mainConfigPath}.flowstate-backup`
      : undefined,
...
async function generateWrapperConfig(
  ...
  const homeDir = os.homedir();
  const currentPresetPath = `${homeDir}/Library/Application Support/Flowstate/current/presets/${appName}`;
  const themePath = `${homeDir}/Library/Application Support/Flowstate/current/theme`;

  if (appName === 'sketchybar') {
    const content = `#!/bin/bash
# Flowstate SketchyBar Configuration
# This file is auto-generated by Flowstate. Customize via overrides.sh instead.
...
`;
    ...
    if (activePreset) {
      ...
      if (existsSync(presetTomlPath)) {
        ...
        if (!content.includes('Flowstate')) {
          // Add after-startup-command to source borders script
          const bordersCommand = `# Flowstate: Apply themed window borders on startup
...
`;
    ...
    if (activePreset) {
      ...
      if (existsSync(presetConfigPath)) {
        ...
        const content = `# Flowstate Starship Configuration
# This file is auto-generated by Flowstate.
# Preset: ${activePreset}
#
# To customize, edit this file directly or create ~/.config/starship-overrides.toml
# and manually source it in your shell init (starship doesn't support includes yet).

${presetContent}`;
        await writeFile(path.join(configDir, 'starship.toml'), content);
        logger.info('Generated Starship config from preset');
      }
    }
  } else if (appName === 'tmux') {
    // tmux uses source-file directive
    const content = `# Flowstate tmux Configuration
# This file is auto-generated by Flowstate. Customize via .tmux-overrides.conf instead.
...
`;
    ...
    if (activePreset) {
      ...
      if (existsSync(presetConfigPath)) {
        ...
        // Prepend Flowstate header
        content = `# Flowstate bat Configuration
# This file is auto-generated by Flowstate.
# Preset: ${activePreset}

${content}`;
        await writeFile(path.join(configDir, 'config'), content);
        logger.info('Generated bat config from preset');
      }
    }
  } else if (appName === 'delta') {
    // delta configures via .gitconfig [delta] section
    await generateDeltaGitconfig(configDir, currentPresetPath);
    logger.info('Generated delta gitconfig section');
  }
}

/**
 * Generate delta configuration by merging into .gitconfig
 */
async function generateDeltaGitconfig(
  configDir: string,
  currentPresetPath: string
): Promise<void> {
  const gitconfigPath = path.join(configDir, '.gitconfig');
  const activePreset = await getActivePreset('delta');

  if (!activePreset) {
    logger.warn('No active delta preset found');
    return;
  }
  ...
  const removeAllSections = (config: string, sectionName: string): string => {
    ...
    // Also removes preceding Flowstate comment if present
    const pattern = new RegExp(
      `(# Flowstate[^\\[]*)?\\[${sectionName}\\][^\\[]*`,
      'gi'
    );
  ...
  // Append the delta sections with Flowstate header
  const flowstateHeader = `# Flowstate delta Configuration
# Preset: ${activePreset}
`;
  newConfig = newConfig.trimEnd() + '\n\n' + flowstateHeader + presetToAdd.trim() + '\n';
