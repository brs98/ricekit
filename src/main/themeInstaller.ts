...
import { generateWeztermConfig } from '../core/apps/adapters/wezterm';
import { generateNeovimConfig } from '../core/apps/adapters/neovim';
import { generateSketchybarConfig } from '../core/apps/adapters/sketchybar';
import { generateAerospaceBordersConfig } from '../core/apps/adapters/aerospace';
import { getAdaptersWithThemeConfig } from '../core/apps/registry';

// Ensure all adapters are registered
import '../core/apps/adapters';
...
# source-file "~/Library/Application Support/Ricekit/current/theme/tmux-colors.conf"
#
...
export async function generateThemeConfigFiles(themeDir: string, metadata: ThemeMetadata): Promise<void> {
  ...
  // Generate adapter-owned config files
  for (const adapter of getAdaptersWithThemeConfig()) {
    const output = adapter.generateThemeConfig!(metadata.colors);
    await writeFile(path.join(themeDir, output.fileName), output.content);
  }

  // Generate standalone config files (not yet migrated to adapters)
  await writeFile(path.join(themeDir, 'alacritty.toml'), generateAlacrittyConfig(metadata.colors));
  await writeFile(path.join(themeDir, 'kitty.conf'), generateKittyConfig(metadata.colors));
  await writeFile(path.join(themeDir, 'iterm2.itermcolors'), generateIterm2Config(metadata.colors));
  await writeFile(path.join(themeDir, 'warp.yaml'), generateWarpConfig(metadata.colors));
