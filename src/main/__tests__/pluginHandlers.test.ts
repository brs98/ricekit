  ...
  describe('handleGetPluginConfig', () => {
    ...
    it('should return config when it exists in preferences', async () => {
      vi.mocked(handleGetPreferences).mockResolvedValueOnce({
        ...
        favorites: [],
        recentThemes: [],
        keyboardShortcuts: { quickSwitcher: 'Cmd+Shift+T', cycleWallpaper: 'Cmd+Shift+W' },
        startAtLogin: false,
        showInMenuBar: true,
...
describe('handleInstallPlugin', () => {
  ...
  it('should throw error and open brew.sh when Homebrew is not installed', async () => {
    ...
    await expect(handleInstallPlugin(null, 'tmux')).rejects.toThrow(
  ...
  it('should run brew install with correct command for tmux', async () => {
    ...
    await handleInstallPlugin(null, 'tmux');
